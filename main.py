...
interface = Interface() ##인터페이스 객체 생성
...
if user_input[0] == 0:#training인 경우
    exercise = Training(user_input[1], user_input[2], user_input[3])#(set, set당 개수, 쉬는시간)
else:
    exercise = Test(user_input[1], user_input[2])#(목표시간, 목표개수)
